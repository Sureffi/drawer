...
// the wrong drawing. It is three constants now, and the zero value is
// auto — which is what -render already defaulted to, so a run nobody
// filled in reads the terminal.
...
// reads the terminal; the two others name themselves.
type rung uint8
